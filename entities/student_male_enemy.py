...
class StudentMaleEnemy(Enemy):
    def __init__(self, x, y):
        ...
        self.preferred_min_distance = 4
        self.preferred_max_distance = 7

        self.throw_cooldown = 0
        self.throw_cooldown_max = 95

        self.is_charging_throw = False
        self.throw_charge_timer = 0
        self.throw_charge_time = 34

        self.throw_speed = 4.8
        self.throw_damage = 2

        self.image = pygame.image.load(
            "assets/student_male_front.png"
        ).convert_alpha()
    ...
    def update(
        ...
        if self.is_charging_throw:
            self.update_throw_charge(player, game_map, projectile_manager)
            return

        if projectile_manager is not None:
            self.try_start_throw(player, game_map)
    ...
    def try_start_throw(self, player, game_map):
        if self.throw_cooldown > 0:
            return
        ...
        self.throw_charge_timer = self.throw_charge_time

    def update_throw_charge(self, player, game_map, projectile_manager):
        self.throw_charge_timer -= 1

        if self.throw_charge_timer > 0:
            return

        self.is_charging_throw = False

        if projectile_manager is not None and self.has_line_of_sight(player, game_map):
            self.throw_phone(player, projectile_manager)

        self.throw_cooldown = self.throw_cooldown_max

    def throw_phone(self, player, projectile_manager):
        ...
        vx = dx / distance * self.throw_speed
        vy = dy / distance * self.throw_speed
